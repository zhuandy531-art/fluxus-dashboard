...
export default function FilterPanel({ filters, onChange, onSearch }) {
  ...
  return (
    <div className="border border-stone-200 rounded-lg mb-4 bg-white">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-4 py-2 text-xs font-medium uppercase tracking-wide text-stone-500"
      >
      ...
      {open && (
        <div className="px-4 pb-4 grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="space-y-3">
            <div className="text-xs font-semibold text-stone-700 uppercase">Ticker</div>
            ...
            <div className="text-xs font-semibold text-stone-700 uppercase mt-3">Basic Info</div>
            <label className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-stone-600">Market Cap</span>
            </label>
            <div className="flex items-center gap-1 text-xs text-stone-500">
              ...
              <input type="number" value={filters.marketCapMin ?? 1}
                ...
                className="border border-stone-300 rounded px-1.5 py-0.5 text-xs w-16 text-center" step="0.1" />
            </div>
            ...
            <label className="flex items-center gap-1.5">
              <input type="checkbox" checked={filters.vol50dEnabled ?? true}
                ...
                className="rounded border-stone-300" />
              <span className="text-xs text-stone-600">Volume (M)</span>
            </label>
            <div className="flex items-center gap-1 text-xs text-stone-500">
            ...
            <label className="flex items-center gap-1.5">
              <input type="checkbox" checked={filters.excludeHealthcare ?? false}
                ...
                className="rounded border-stone-300" />
              <span className="text-xs text-stone-600">Exclude Healthcare</span>
            </label>
          </div>

          {/* Column 2: Changes % */}
          <div className="space-y-2">
            <div className="text-xs font-semibold text-stone-700 uppercase">Changes %</div>
          ...
          <div className="space-y-2">
            <div className="text-xs font-semibold text-stone-700 uppercase">Distance from MA</div>
          ...
          <div className="space-y-2">
            <div className="text-xs font-semibold text-stone-700 uppercase">Volatility</div>
            <RangeFilter label="ADR %" filterKey="adrPct" value={filters.adrPct} onChange={(k,v) => updateFilter(k,v)} />

            <div className="text-xs font-semibold text-stone-700 uppercase mt-4">RS Scores</div>
            ...
            <div className="text-xs font-semibold text-stone-700 uppercase mt-4">Other</div>
